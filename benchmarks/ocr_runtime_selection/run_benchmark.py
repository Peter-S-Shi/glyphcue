...
- Each engine is constructed once per language configuration needed,
  and warmed up with one throwaway inference call before timing begins
  (first-call latency includes JIT/session setup and is reported
  separately as "startup").
- Per-item latency is the median of 3 timed calls after warm-up.
- Peak process RSS is sampled via psutil immediately after
  construction (model load) and again after the heaviest recognition
  call, both reported as deltas from a baseline taken before import.
...
def benchmark_paddleocr() -> dict:
    ...
    default_engine.predict(warmup_image)  # warm-up call, discarded
    ...
    return {
        ...
        "memory_after_load_delta_mb": round(after_load_mb - baseline_mb, 1),
        "memory_peak_mb": round(peak_mb, 1),
        "languages_requiring_separate_model_load": sorted(languages_loaded),
        "per_item": per_item,
    }
