...
**Status (2026-08-31): M10 is complete per ROADMAP.md §17's gate audit
disposition.** One target — the representative-video corpus (item 2,
§10 below) — was not completed and is transferred to Milestone 11 as a
mandatory acceptance gate, not waived. See `ROADMAP.md` §17/§18 for the
full disposition and transfer rationale.
...
2. **Representative-video corpus** (3–5 videos × 2–5 min) — **not completed in M10; transferred to Milestone 11 as a mandatory gate, per the M10 gate audit disposition (ROADMAP.md §17, 2026-08-31).** The realistic private-corpus attempt produced partial external-realistic evidence but did not complete (crashed on a harness bug before any entry finished; see `docs/m10_private_corpus_incident.md` for what happened and what real evidence was recovered, and `docs/m10_performance_diagnosis.md` for the isolated bottleneck analysis + ranked M11 candidates it motivated). The controlled/synthetic corpus in `benchmarks/m10_controlled_video_corpus/` closes only the reproducible performance-diagnosis seam — it does not satisfy ROADMAP §17's 3–5 representative videos × 2–5 minute segments target, and is not presented as equivalent to it. **This target is not waived, downgraded to optional debt, or silently moved** — M10's gate audit accepted M10 as complete while explicitly transferring completion of this item to Milestone 11 (ROADMAP.md §18's acceptance gate 9), because the crashed attempt exposed exactly the Path A performance/harness prerequisites M11 Product Hardening is responsible for resolving. M12 must not begin until Milestone 11 completes this evaluation and folds its results back into `EVALUATION_REPORT.md` and, where relevant, `FAILURE_MODE_REPORT.md`.
...
- **Observed failure taxonomy** — done: `FAILURE_MODE_REPORT.md` (repo root), preceded by the minimal `BUILD_VS_INTEGRATE.md` precursor it needs to classify each finding as a dependency, GlyphCue-orchestration, GlyphCue-reconstruction, evaluation-harness, or evidence/corpus limitation. 11 entries, each pointing to an already-committed artifact (no invented categories): Review Priority's `low_confidence_only` miss and overall parity result, Path B's by-design non-resolution of ambiguous cases, the Path B corpus's non-held-out scope, the multilingual missing/wrong-layer evidence gap, the private-corpus harness crash, the real-world OCR-trigger-rate finding, PaddleOCR's per-call latency cost, the noisy-background reproduction negative result, ADR 0005's un-benchmarked evidentiary basis, and two resolved M3 dependency-level failures (PaddleOCR mkldnn crash, RapidOCR's Japanese failure). The representative-video acceptance item is explicitly transferred to Milestone 11 as a mandatory gate (see item 2 above), not treated as closed by any finding in it.
