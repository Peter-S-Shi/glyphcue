...
2. **Representative-video corpus** (3–5 videos × 2–5 min) — **open, not closed.** The realistic private-corpus attempt produced partial external-realistic evidence but did not complete (crashed on a harness bug before any entry finished; see `docs/m10_private_corpus_incident.md` for what happened and what real evidence was recovered, and `docs/m10_performance_diagnosis.md` for the isolated bottleneck analysis + ranked M11 candidates it motivated). The controlled/synthetic corpus in `benchmarks/m10_controlled_video_corpus/` closes only the reproducible performance-diagnosis seam — it does not satisfy ROADMAP §17's 3–5 representative videos × 2–5 minute segments target, and is not presented as equivalent to it. **This target remains an explicit M10 limitation / open acceptance item**, to be dispositioned at the M10 gate audit rather than silently waived or moved.
