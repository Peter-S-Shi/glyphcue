...
This ADR rejected RapidOCR as the **default** runtime (above), but explicitly left the door open ("What remains swappable") for a second `OcrEngine`/`RegionOcrEngine` implementation if real evidence justified one. Milestone 11's P3 gate (private evidence: `private_samples/phase0b/PHASE0B_REPORT.md`, not tracked) measured RapidOCR + ONNX Runtime DirectML (`DmlExecutionProvider`) as a genuine GPU accelerator option and found real upside (fixed-crop recognition-only latency 0.184–0.294s vs Paddle's CPU path) alongside real, unresolved risk: RapidOCR's bundled small recognition model shows non-zero CER against Paddle's zero-CER result on at least one sample, and Chinese/Japanese coverage in that evidence is a single synthetic case each — not enough to support a broad accuracy claim. The gate's own conclusion (risk: medium-to-high; priority: medium) was **opt-in only, keep the `OcrEngine` contract, do not change the default** — this addendum implements exactly that, nothing more.

**What shipped:** `glyphcue.adapters.directml_ocr_engine.DirectMlOcrEngine`, a `RegionOcrEngine` implementation identical in shape to `PaddleOcrEngine`'s P2 recognition-only path — it uses RapidOCR's own public standalone method (`RapidOCR.recognize_txt`, which only calls `self.text_rec`, never `self.text_det`) for `recognize_regions`, and reuses GlyphCue's own `_sort_polygons_in_reading_order`/`_crop_polygon_region` (the same functions `PaddleOcrEngine` uses) rather than any RapidOCR-internal geometry helper. `glyphcue.adapters.ocr_engine_selection.create_ocr_engine(language, prefer_directml=...)` is the only sanctioned construction path: `prefer_directml` defaults to `False` (existing callers are unaffected), and even when a caller opts in, `PaddleOcrEngine` is returned unless a real DirectML initialization probe on this machine succeeds — a missing `[directml]` install, a non-Windows platform, or a genuine provider-initialization failure (e.g. no DX12-capable adapter) all fall back to Paddle, not to a crash.
