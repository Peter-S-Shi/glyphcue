...
Two artifact-truth corrections folded into this count: the corpus was missing the backtrack/pure-suffix repetition case ("Hello world, how are you" -> "how are you", the bounded-gap repetition already covered by a unit regression) -- it is now included, bringing `rolling_reconstruction` to 8 (English 5/5) and the total to 17. And `Case` gained an optional `expected_spans` check so text/flag assertions alone can't mask a timing-normalization regression -- the clean-preservation and irregular-timing-span cases now assert the real `(start_time, end_time)`, with the irregular case specifically verifying the merged span is `(0.0, 5.0)` (the latest end_time across supporting evidence, not the last Observation's own end). `tests/application/test_path_b_normalization_evaluation.py` locks these counts in as a regression.
